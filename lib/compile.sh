...
# Rscript -e "options(unzip='$(which unzip)'); Sys.setenv(TAR='$(which tar)'); source('https://bioconductor.org/biocLite.R'); BiocInstaller::biocLite(c('BiocParallel','genefilter','DESeq2','TCGAutils','TCGAbiolinks'), ask=F, Ncpus=$threads, clean=T)"
# from bioconductor R >= 3.5
# Rscript -e "options(unzip='$(which unzip)'); Sys.setenv(TAR='$(which tar)'); BiocManager::install(c('BiocParallel','genefilter','DESeq2','TCGAutils','TCGAbiolinks'), ask=F, Ncpus=$threads, clean=T)"
