...
function quantify::bamcoverage(){
	...
	function _usage(){
		commander::print {COMMANDER[0]}<<- EOF
			...
			-d              | when using -w <windowsize>, use deepTools bamCoverage (ignores -O -f -u -p)
		EOF
		return 1
	}

	local OPTIND arg mandatory skip=false tmpdir="${TMPDIR:-/tmp}" threads overlap=1 windowsize=100 bed index=0 fractional=false unambiguous=false pairs=false outdir
	declare -n _mapper_bam2bedg _strandness_bam2bedg
	while getopts 'S:s:t:r:x:o:O:w:b:ifup' arg; do
		case $arg in
			S) $OPTARG && return 0;;
			s) $OPTARG && skip=true;;
			t) ((++mandatory)); threads=$OPTARG;;
	...
	for m in "${_mapper_bam2bedg[@]}"; do
		...
		for f in "${_bams_bam2bedg[@]}"; do
			...
			if [[ $bed || $windowsize -gt 1 ]]; then
				...
				CMD
					for r in "\${chr[@]}"; do
						bamCoverage -r \$r -b "$f" -bs $windowsize -p $threads --normalizeUsing None --outFileFormat bedgraph -o "${tdirs[-1]}/$e.counts.bed";
				...
				CMD
					for r in "\${chr[@]}"; do
						bamCoverage -r \$r -b "$f" -bs $windowsize -p $threads --normalizeUsing CPM --outFileFormat bedgraph -o "${tdirs[-1]}/$e.tpm.bed";
