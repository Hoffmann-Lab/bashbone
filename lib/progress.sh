...
function progress::_log(){
	...
	case $verbosity in
		...
		1)	tee -ia "$log" < "$fifo" | sed -u -nE '/^\s*(:INFO:|:CMD:|:BENCHMARK:|:WARNING:)/p;/^\s*:CMD[0-9]+:\s*start/,/^\s*:CMD[0-9]+:\s*end/p' || true;;
		# 2)	tee -ia "$log" < "$fifo" | grep --color=never -E --line-buffered '^\s*:ERROR:' || true;;
		2)	tee -ia "$log" < "$fifo" | sed -u -n '/^\s*:ERROR:/p' || true;;
		*)	_usage;;
	esac
