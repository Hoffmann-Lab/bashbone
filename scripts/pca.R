...
if(length(args)<3){
  cat("pca 1vs2, 1vs3, 2vs3 from up to top 2000 most variable features\n")
  cat("\n")
...
n=2000
vars = order(rowVars(df), decreasing = TRUE)
pca = prcomp(t(df[vars[1:n], ]), scale = F)
...
ggplot(data, aes(PC1, PC2, color = condition, shape = factor, label=sample)) +
...
ggplot(data, aes(PC1, PC3, color = condition, shape = factor, label=sample)) +
  ggtitle(paste0("PCA of top ",n," variable features")) +
  theme_bw() +
  theme(aspect.ratio=1, legend.box = "horizontal", legend.title=element_blank()) +
  geom_point(size = 3) +
...
ggplot(data, aes(PC2, PC3, color = condition, shape = factor, label=sample)) +
  ggtitle(paste0("PCA of top ",n," variable features")) +
  theme_bw() +
  theme(aspect.ratio=1, legend.box = "horizontal", legend.title=element_blank()) +
  geom_point(size = 3) +
